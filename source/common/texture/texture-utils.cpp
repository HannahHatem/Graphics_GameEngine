...
glm::ivec2 our::texture_utils::loadImage(Texture2D& texture, const char *filename, bool generate_mipmap) {
    ...
    texture.bind();
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8,size.x,size.y,0,GL_RGBA, GL_UNSIGNED_BYTE,(void*)data); // sending the data to the V ram
    if(generate_mipmap)glGenerateMipmap(GL_TEXTURE_2D); // CALCULATING THE AVERAGE OF COLORS IN A GROUP OF PIXELS
